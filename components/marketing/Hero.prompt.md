The signature OncoPath hero: private-dining photo behind a navy gradient scrim, lane-tinted far edge.

```jsx
<Hero
  lane="fellows"
  eyebrow="Oncology fellows community"
  title="OncoPath Fellows Exchange"
  copy="Physician-led small-group community for oncology subspecialty fellows."
  image="assets/imagery/oncopath-hero.png"
  actions={<>
    <Button as="a" href="./events.html#event-register">Request a Seat</Button>
    <Button variant="secondary" as="a" href="./events.html">View Upcoming Events</Button>
  </>}
/>
```

Set `image` to the correct relative path from the page using the hero. Lane controls scrim edge color + eyebrow.
